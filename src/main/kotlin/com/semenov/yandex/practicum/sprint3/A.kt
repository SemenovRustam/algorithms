...
fun main() {
    ...
    val sequences = generateParenthesis(max)
    sequences.forEach { println(it) }
}


private fun generateParenthesis(max: Int) = buildList {
    generate(this, "", 0, 0, max)
}

private fun generate(result: MutableList<String>, s: String, open: Int, close: Int, max: Int) {
    if (s.length == max * 2) {
        result.add(s)
        return
    }

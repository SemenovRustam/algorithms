...
//todo  решить более эффективным способом
fun main() {
    readln().toInt()
    val landsSize = readln().split(" ").map { it.toInt() }
    val k = readln().toInt()

    println(getLandDiff(landsSize, k))
}

fun getLandDiff(landsSize: List<Int>, k: Int): Int {
    val diffs = buildList {
        for (i in landsSize.indices) {
            for (j in i + 1 until landsSize.size) {
                val abcDiff = abs(landsSize[i] - landsSize[j])
                add(abcDiff)
            }
        }
    }

    return diffs.sorted()[k - 1]
}
...
Для этого Гоша рассмотрел все пары островов (таких пар, напомним, n * (n-1) / 2)
...
В следующей строке через пробел записаны n площадей островов — n натуральных чисел,
каждое из которых не превосходит 1 000 000.

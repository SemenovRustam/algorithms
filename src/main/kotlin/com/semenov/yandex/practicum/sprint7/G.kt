...
fun main() {
    val m = readln().toInt()
    readln()
    val nominal = readln().split(" ").map { it.toInt() }

    println(countCoinsWays(m, nominal))
}

fun countCoinsWays(m: Int, coins: List<Int>): Int {
    val dp = IntArray(m + 1)
    dp[0] = 1
...
В банкомате в бесконечном количестве имеются купюры различных достоинств. Всего различных достоинств n.
Купюр каждого достоинства можно взять бесконечно много. Нужно определить число способов,
